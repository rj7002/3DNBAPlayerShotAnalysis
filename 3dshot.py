...
# #         # Save to CSV
# #         df_plays.to_csv(output_file, index=False,mode='a')

# #         print(f"Successfully saved play-by-play data to {output_file}")
# #     except Exception as e:
# #         print(f"Error fetching or saving play-by-play data: {e}")

# # with open('all_pbps.csv', 'w', newline='') as csvfile:
# #     csvfile.truncate()
# # season = st.slider('Select a season',2003,2024)
# # seasons = range(2002, season)

# # # Load NBA player boxscore data for the specified seasons
# # nba_df = sda.nba.load_nba_player_boxscore(seasons=seasons)
# # athlete_names = nba_df['athlete_display_name']

# # # Convert the Polars Series to a Python list
# # players = athlete_names.to_list()
# # player = st.selectbox('Select a player',[''] + players)
# # filtered_df = nba_df.filter(pl.col('athlete_display_name') == player)

# # # Define the output CSV file path
# # output_file2 = "all_pbps.csv"
...
# # Loop through game_ids in filtered_df and fetch/save PBP data
# # count = 0
# # for game_id2 in filtered_df['game_id']:
# #     fetch_and_save_nba_pbp(game_id=game_id2, output_file=output_file2)
# #     count += 1
# #     print(count)
# #     print(f'ID: {game_id2}')

# df2 = pd.read_csv('all_pbps.csv')

# # # # Filter the DataFrame
# # df_filtered = df[df['text'].str.split('.').str[0].str.contains(player, case=False, na=False)]# # # Additional filters if needed
# # df_filtered = df_filtered[df_filtered['scoringPlay'] == True]
# # df_filtered = df_filtered[~df_filtered['type.text'].str.contains('free throw', case=False, na=False)]

# # # # Save the filtered DataFrame to CSV without index
# # df_filtered.to_csv('all_pbps_filtered.csv', index=False)

# # df2 = pd.read_csv('all_pbps_filtered.csv')
# team_id = df2['homeTeamId'][1]
...
# df2['team'] = df2.apply(label_team, axis=1)
# df2.to_csv('all_pbps_filtered.csv')
# df2 = pd.read_csv('all_pbps_filtered.csv')
...
#     symbol='shot_made',
#     symbol_map={'made': 'circle-open'}
# )

# hovertemplate= '%{customdata[0]}<br>%{customdata[2]} - %{customdata[3]}<br>%{customdata[1]}'
# shot_path_fig.update_traces(opacity=0.55, hovertemplate=hovertemplate, showlegend=False,marker_size=3)
...
with co2:
    ...
    # first_season = int(first_season)
    # last_season = int(last_season)
    # selected_season = st.selectbox('Select a season', range(first_season,last_season))
    # nextseason = selected_season+1
    # nextstr = str(nextseason)[2:]
    # realseason = str(selected_season) + "-" + nextstr
    # court = CourtCoordinates(realseason)
    ...
    # #         x=[-row['LOC_X']],
    # #         y=[-row['LOC_Y']+300],
    # #         z=[0],  # Ensure z is a list with a single element
    # #         mode='markers',
    ...
    #             data=[go.Scatter3d(x=x[:int(len(x))], y=y[:int(len(y))], z=z[:int(len(z))],
    ...
    #     x2 = court.hoop_loc_x
    #     y2 = court.hoop_loc_y
    #     p1 = np.array([x1, y1, 0])  # Start point (on the floor)
    #     p2 = np.array([x2, y2, 100])  # End point (basket height)
    ...
    #         for t in np.linspace(0, 1, 10):  # Adjust the number of frames for smoothness
    ...
    #                     #                mode='lines', line=dict(width=8, color='blue'))
    #                     #                ,
    #                                    go.Scatter3d(x=frame_x, y=frame_y, z=frame_z,
    #                                    mode='markers', line=dict(width=6, color='green'))],
    #                 name=f'Frame {i}_{int(t*100)}'
    #             ))

    # # Update the existing figure with animation controls
    ...
    #         'buttons': [
    #             {
    #                 'args': [None, {'frame': {'duration': 0.5, 'redraw': True}, 'fromcurrent': True}],
    #                 'label': 'Play',
    #                 'method': 'animate'
    #             },
    #             {
    ...
    #     }],
    #     scene=dict(
    #         xaxis=dict(title='X', showticklabels=False, showgrid=False),
    #         yaxis=dict(title='Y', showticklabels=False, showgrid=False),
    #         zaxis=dict(title='Z', showticklabels=False, showgrid=False, showbackground=True, backgroundcolor='#d2a679')
    #     ),
    #     height=600
    # )
